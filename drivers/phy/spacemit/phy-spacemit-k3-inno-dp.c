 ...
 *
 * The PHY registers are interleaved with the DP controller's inside one MMIO
 * window, so this driver probes as its child and shares the parent's regmap.
 */
...
/* Offsets are into the parent controller's MMIO window, not PHY-local. */
...
/* Four lanes in one word: lane N occupies bits N*6+3 .. N*6+0, with a 2-bit gap. */
...
/* MPLL is the link PLL (lane rate); PREPLL is the pixel PLL exposed as a clock. */
...
#define DPTX_PLL_FRAC_MOD		16777216	/* 2^24 */
...
struct k3_inno_dp_phy {
	...
	unsigned long	pxclk_rate;	/* what it was last programmed to */
...
/*
 * The MPLL is left running on purpose: AUX shares its analog block and must keep
 * answering .detect()/.edid_read() while the display is down.
 */
...
/* The VCO is shared: 5.4 Gbps simply stops dividing it down. */
					...
					const struct k3_inno_dp_mpll_cfg *cfg)
{
	...
	/*
	 * The analog block is shared with an unwired HDMI path. Point it at DP first,
	 * or the PLLs run, nothing reaches the lanes, and even AUX times out.
	 */
...
/* Drive the source TX at the level the sink's adjust request asked for. */
...
static int k3_inno_dp_prepll_div_total(const struct k3_inno_dp_prepll_cfg *cfg,
				       u32 *div_total)
{
	static const u8 divm_factors[] = { 1, 2, 3, 5 };

	if (!cfg || !div_total || !cfg->valid)
		return -EINVAL;

	/* div5_en hard-codes div_total=5 and legitimately carries divp == 0. */
	if (cfg->div5_en) {
		*div_total = 5;
		return 0;
	}
					 ...
					 u32 ref_clk_khz)
{
	...
	actual_pclk_khz = k3_inno_dp_pll_rate_khz(cfg->prediv, cfg->fbdiv, cfg->frac,
				  ...
				  struct k3_inno_dp_prepll_cfg *best)
{
	...
	actual_pclk = k3_inno_dp_pll_rate_khz(pre, fb, frac, ref_clk_khz, div_total);
...
{
	static const u8 divm_factors[] = {1, 2, 3, 5};
	static const u8 divm_regs[]    = {0, 1, 2, 3};
	...
	for (pclk_div = 1; pclk_div <= 31; pclk_div++) {
		/* Strategy 2: DivM path (divaux must be 1 to enable DivM) */
		for (i = 0; i < ARRAY_SIZE(divm_factors); i++)
			k3_inno_dp_try_prepll(target_pclk_khz, ref_clk_khz,
					      2 * divm_factors[i] * pclk_div,
					      0, 1, divm_regs[i], pclk_div,
					      &best);

		/* Strategy 3: DivAux path (divaux > 1; divm is ignored) */
		for (i = 2; i <= 31; i++)
			k3_inno_dp_try_prepll(target_pclk_khz, ref_clk_khz,
					      2 * i * pclk_div,
					      0, i, 0, pclk_div, &best);
				      ...
				      const struct k3_inno_dp_prepll_cfg *cfg)
{
	...
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL, DPTX_ANA_PREPLL_FBDIV2_LBIT,
			  FIELD_PREP(DPTX_ANA_PREPLL_FBDIV2_LBIT, cfg->fbdiv & 0xFF));
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL, DPTX_ANA_PREPLL_FBDIV2_HBIT,
	...
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_FRAC, DPTX_ANA_PREPLL_FRAC2_LBIT,
			  FIELD_PREP(DPTX_ANA_PREPLL_FRAC2_LBIT, cfg->frac & 0xFF));
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_FRAC, DPTX_ANA_PREPLL_FRAC2_MBIT,
			  FIELD_PREP(DPTX_ANA_PREPLL_FRAC2_MBIT, (cfg->frac >> 8) & 0xFF));
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_FRAC, DPTX_ANA_PREPLL_FRAC2_HBIT,
			  FIELD_PREP(DPTX_ANA_PREPLL_FRAC2_HBIT, (cfg->frac >> 16) & 0xFF));

	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_DIV, DPTX_ANA_PREPLL_PRECLK_DIVM,
			  FIELD_PREP(DPTX_ANA_PREPLL_PRECLK_DIVM, cfg->divm));
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_DIV, DPTX_ANA_PREPLL_PRECLK_DIVAUX,
			  FIELD_PREP(DPTX_ANA_PREPLL_PRECLK_DIVAUX, cfg->divaux));

	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_CTRL, DPTX_ANA_PREPLL_PCLKDIV5_EN,
			  FIELD_PREP(DPTX_ANA_PREPLL_PCLKDIV5_EN, cfg->div5_en));
	regmap_write_bits(p->regmap, DPTX_ANA_PREPLL_CTRL, DPTX_ANA_PREPLL_PCLK_DIVAUX,
...
/*
 * Expose the PREPLL as a clock so the pixel-clock topology lives in the clock
 * framework instead of a display driver reaching into PHY registers.
 */
#define to_k3_inno_dp_phy_clk(_hw) container_of(_hw, struct k3_inno_dp_phy, pxclk_hw)
...
static int k3_inno_dp_phy_probe(struct platform_device *pdev)
{
	...
	/*
	 * The 24 MHz reference is not a binding clock input: the PHY has no clock
	 * line of its own, the reference reaches the analog block internally.
	 */
	p->ref_clk_khz = 24000;
