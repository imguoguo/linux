	...
	drm_atomic_crtc_state_for_each_plane_state(plane, pstate, crtc_state) {
		...
		if (spacemit_pstate->use_scl) {
			...
			if (i == MAX_SCALER_NUMS) {
				drm_dbg_kms(crtc->dev, "Exceeds the max scaler number\n");
				return -EINVAL;
			}
		}
	}
					...
					struct drm_atomic_commit *old_state)
{
	 ...
	 * resets alone; the clocks are not ours to gate until we have enabled them.
	 */
	...
	if (!a_crtc->power_on) {
		...
		/* The DPU must settle after reset deassert or CFG_RDY races the flush. */
		msleep(10);
	}
					 ...
					 struct drm_atomic_commit *old_state)
{
	...
	/*
	 * MMIO on a clock-gated DPU stalls the bus, so an in-flight handler must
	 * finish its status reads before dpu_pm_suspend() gates the clocks.
	 */
				      ...
				      struct drm_atomic_commit *atomic_state)
{
	...
	if (ret) {
		drm_dbg_kms(crtc->dev, "crtc_id=%u check_scaling FAILED ret=%d\n",
			    crtc->base.id, ret);
		return ret;
	}
	...
	if (ret) {
		drm_dbg_kms(crtc->dev, "Failed to satisfy fbcmem size for all rdmas!\n");
		return ret;
	}

	saturn_check_dpuctrl_scl_reuse(crtc, state);

	ret = spacemit_cmdlist_alloc(a_crtc->dev, &cstate->cl[cstate->cur_cl]);
	if (ret)
		return ret;
		...
		for (i = 0; i < MAX_CL_NUM; i++) {
			if (spacemit_state->cl[i].va)
				dma_free_coherent(a_crtc->dev, spacemit_state->cl[i].size,
						  spacemit_state->cl[i].va,
						  spacemit_state->cl[i].pa);
		}
				   ...
				   struct drm_plane *primary, struct device_node *port)
{
	...
	/* drm_of_find_possible_crtcs() walks crtc->port for the life of the drm_device. */
					  ...
					  struct platform_device *pdev)
{
	...
	priv->mclk_reset = devm_reset_control_get_optional_exclusive(dev, "mclk");
	...
	priv->aclk_reset = devm_reset_control_get_optional_exclusive(dev, "aclk");
	...
	/*
	 * Keep the line disabled until bind: the handler needs bind-created state,
	 * and a bootloader-left display can raise an interrupt during probe.
	 */
	ret = devm_request_irq(dev, priv->irq_online, spacemit_dpu_isr,
			       IRQF_NO_AUTOEN, "DPU_ONLINE", priv);
	if (ret)
		return dev_err_probe(dev, ret, "failed to request online IRQ\n");
...
static int spacemit_drm_master_bind(struct device *dev)
{
	...
	if (ret) {
		dev_err(dev, "failed to bind all component.\n");
		goto err_dc;
	}
...
err_ports_put:
	...
	of_node_put(ports);
err_dc:
	/* .shutdown tests priv->ddev; never leave it on a half-initialised device. */
	priv->ddev = NULL;
	priv->a_crtc = NULL;
	return ret;
}
...
static int spacemit_dpu_probe(struct platform_device *pdev)
{
	...
	priv->hwdev->base = devm_platform_get_and_ioremap_resource(pdev, 0, &r);
...
/* The assert and deassert orders are not mirror images: the hardware wants these. */
static void dpu_reset_assert(struct device *dev, const char *name,
			     struct reset_control *rstc)
{
...
static int dpu_pm_suspend(struct device *dev)
{
	...
	/*
	 * Assert before gating: a reset asserted into an already-gated block has
	 * no clock edges to propagate on and leaves the register file untouched.
	 */
