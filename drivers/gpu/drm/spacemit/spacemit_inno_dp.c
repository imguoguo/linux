...
/* A sink may hold HPD low far longer than the DP spec's 2 ms disconnect threshold. */
#define SPACEMIT_DP_HPD_POLL_US     1000
#define SPACEMIT_DP_HPD_DEBOUNCE_US (100 * USEC_PER_MSEC)
...
static int spacemit_dp_get_bpp(u32 format)
{
...
/**
 * struct spacemit_dp_dev - Per-instance state for an Innosilicon DP/eDP controller
 * @dev: Underlying platform device.
 * @drm: Parent DRM device, set at component bind.
 * @encoder: Passive encoder anchoring possible_crtcs; atomic goes through @bridge.
...
struct spacemit_dp_dev {
	...
	struct clk *pxclk;
	struct clk *pll_clk;	/* the PHY's pixel PLL; clk_set_rate() programs it */
	struct phy *phy;
	bool phy_powered;
	...
	int pixel_clock;
	struct mutex mode_lock;	/* serializes mode/link programming vs HPD */
 ...
 * commit already holds: calling it from .atomic_enable deadlocks against itself.
 */
...
{
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_AUX_RESET, FIELD_PREP(DP_AUX_RESET, 0x1));
	usleep_range(2000, 2500);
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_AUX_RESET, FIELD_PREP(DP_AUX_RESET, 0x0));
	usleep_range(2000, 2500);
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT, DP_AUX_REPLY_EVENT_INT_STA,
					...
					struct drm_dp_aux_msg *msg)
{
	...
	struct spacemit_dp_dev *dp = container_of(aux, struct spacemit_dp_dev, aux);
...
retry_eio:
	...
	regmap_write_bits(dp->regs, DP_AUX_CMD_REG, DP_AUX_LENGTH, FIELD_PREP(DP_AUX_LENGTH, len));
	...
	regmap_write_bits(dp->regs, DP_AUX_START_REG, DP_AUX_START, FIELD_PREP(DP_AUX_START, 0));
	regmap_write_bits(dp->regs, DP_AUX_START_REG, DP_AUX_START, FIELD_PREP(DP_AUX_START, 1));
	...
	regmap_read(dp->regs, DP_AUX_STS_REG, &status); status = FIELD_GET(DP_AUX_STATUS, status);
...
static int spacemit_dp_hw_read_sink_caps(struct spacemit_dp_dev *dp)
{
	...
	dp->link.max_rate = min(dp->link.max_rate, (u32)SPACEMIT_DP_SOURCE_MAX_RATE);
	...
	dev_dbg(dp->dev, "DPCD: Rev %x.%x, MaxRate %d kHz, MaxLanes %d, EnhFrame %d\n",
 ...
 * rather than pair-balanced. Mirror the state so the refcount cannot go negative.
 */
...
{
	regmap_write_bits(dp->regs, DP_VIDEO_VSAMPLE_REG, DP_VIDEO_STREAM_ENABLE,
...
static enum drm_connector_status spacemit_dp_hw_detect_hpd(struct spacemit_dp_dev *dp)
{
	u32 hpd_status;
...
static void spacemit_dp_hw_clean_hpd(struct spacemit_dp_dev *dp)
{
	...
	if (plug_event)
		regmap_write(dp->regs, DP_HPD_INTERRUPT_STATUS, FIELD_PREP(DP_HOT_PLUG_EVENT, 0x1));
...
static int spacemit_dp_set_training_pattern(struct spacemit_dp_dev *dp, u8 pattern)
{
	u32 tps_sel = 0;
	...
	ret = drm_dp_dpcd_write_byte(&dp->aux, DP_TRAINING_PATTERN_SET, dpcd_pattern);
	if (ret < 0) {
				  ...
				  enum spacemit_dp_lane_count lanes)
{
	...
	/* A sink may act on the DP_LINK_BW_SET write, so set the lane count first. */
	link_config[0] = lanes;
	if (dp->link.enhanced_framing)
		link_config[0] |= DP_LANE_COUNT_ENHANCED_FRAME_EN;

	ret = drm_dp_dpcd_write_byte(&dp->aux, DP_LANE_COUNT_SET, link_config[0]);
	if (ret < 0) {
		...
		u32 color_format)
{
	...
	/* Get BPP */
	bpp = spacemit_dp_get_bpp(color_format);
	...
	dev_dbg(dp->dev, "MSA: %dx%d, Rate:%d kHz, Lanes:%d, BPP:%d, TU:%d.%d\n",
	...
	if (mode->flags & DRM_MODE_FLAG_PHSYNC)
		...
		regmap_write_bits(dp->regs, DP_VINPUT_POLARITY, DP_HSYNC_IN_POLARITY,
				  FIELD_PREP(DP_HSYNC_IN_POLARITY, 0));
	...
	if (mode->flags & DRM_MODE_FLAG_PVSYNC)
		...
		regmap_write_bits(dp->regs, DP_VINPUT_POLARITY, DP_VSYNC_IN_POLARITY,
				  FIELD_PREP(DP_VSYNC_IN_POLARITY, 0));
	...
	/* MSA and MISC */
	regmap_write_bits(dp->regs, DP_VIDEO_MSA2, DP_MISC0, FIELD_PREP(DP_MISC0, misc0));
	regmap_write_bits(dp->regs, DP_VIDEO_MSA3, DP_MISC1, FIELD_PREP(DP_MISC1, 0));
	regmap_write_bits(dp->regs, DP_VIDEO_MSA3, DP_NVID, FIELD_PREP(DP_NVID, 0));

	/* Link layer parameters */
	regmap_write_bits(dp->regs, DP_VIDEO_HBLANK_INTERVAL, DP_HBLANK_INTERVAL,
	...
	regmap_write_bits(dp->regs, DP_VIDEO_CONFIG5, DP_AVERAGE_BYTES_PER_TU_FRAC,
...
{
	dev_dbg(dp->dev, "Disabling Video & PHY\n");
	...
	/*Disable BG_EN */
	regmap_write_bits(dp->regs, DP_ANA_TX_CTRL6, DP_ANA_BG_EN, FIELD_PREP(DP_ANA_BG_EN, 0));
	regmap_write_bits(dp->regs, DP_ANA_TX_CTRL7, DP_ANA_BG_EN_RCAL,
			  FIELD_PREP(DP_ANA_BG_EN_RCAL, 0));

	/* Both PLLs. */
	regmap_write_bits(dp->regs, DP_MPLL_CTRL0, DP_ANA_MPLL_PD, FIELD_PREP(DP_ANA_MPLL_PD, 1));
...
static u32 spacemit_dp_calc_required_bw(const struct drm_display_mode *mode, int bpp)
{
...
{
	struct spacemit_dp_dev *dp = container_of(bridge, struct spacemit_dp_dev, bridge);
	enum drm_connector_status status;

	/*
	 * The eDP panel is fixed, and DP_HPD_IN_STATUS does not read meaningfully
	 * on that path. Report CONNECTED only once panel-edp has joined the chain.
	 */
	if (dp->edp_mode) {
		if (!dp->next_bridge)
	...
	/*
	 * The HPD interrupt is gated by the DP pixel clock, which is off exactly
	 * when a plug must be caught. Read the level register live instead.
	 */
	guard(mutex)(&dp->mode_lock);
	status = spacemit_dp_hw_detect_hpd(dp);

	/*
	 * The bind-time cap read only covers a sink that was already there. Without
	 * a re-read here, link.max_rate stays 0 and every link config is filtered out.
	 */
					   ...
					   unsigned int block, size_t len)
{
	...
	if (segment) {
		for (retry = 0; retry < 3; retry++) {
			ret = spacemit_dp_aux_i2c_write(dp, SPACEMIT_DP_DDC_SEGMENT_ADDR,
							&segment, 1);
			if (!ret)
				break;
		}
	...
	for (offset = 0; offset < len; offset += SPACEMIT_DP_EDID_CHUNK_SIZE) {
		...
		size_t chunk = min_t(size_t, SPACEMIT_DP_EDID_CHUNK_SIZE, len - offset);

		for (retry = 0; retry < 3; retry++) {
			ret = spacemit_dp_aux_i2c_write(dp, DDC_ADDR, &edid_offset, 1);
			if (ret)
				continue;

			ret = spacemit_dp_aux_i2c_read(dp, DDC_ADDR, buf + offset, chunk);
			if (!ret)
				break;
		}
...
spacemit_dp_bridge_edid_read(struct drm_bridge *bridge, struct drm_connector *connector)
{
	...
	scoped_guard(mutex, &dp->mode_lock) {
		for (retry = 0; retry < SPACEMIT_DP_SINK_READY_RETRIES; retry++) {
			...
			edid = drm_edid_read_custom(connector, spacemit_dp_conn_get_edid_block, dp);
			if (edid)
				break;
		}
	}
			 ...
			 const struct drm_display_mode *mode)
{
	...
	/* drm_dp_max_link_rate() is in 10 kHz units; clamp to what the PHY can do. */
	...
	/*
	 * max_link_rate is per lane in 10 kHz units. The x10 to reach kHz and the
	 * 8b/10b coding overhead cancel down to a plain "* 8".
	 */
					     ...
					     struct drm_atomic_commit *state)
{
	...
	struct spacemit_dp_bridge_state *st = to_spacemit_dp_bridge_state(new_bridge_state);
	...
	bpp = spacemit_dp_get_bpp(st->color_format);
	req_bw = spacemit_dp_calc_required_bw(adjusted_mode, bpp);

	/* Start where atomic_check() left off, but keep the upgrade walk as fallback. */
	...
	for (trained = false, i = start;
		...
		capacity = spacemit_dp_calc_link_capacity(cfg->rate, cfg->lanes);
		if (capacity < req_bw)
			continue;
		...
		/*
		 * Debounce: the transmitter was power-cycled just now and a sink may
		 * legitimately drop HPD across a link-down. eDP is hardwired and noisy.
		 */
		if (!dp->edp_mode &&
			...
			dev_warn(dp->dev, "DP: sink is not asserting HPD; not training\n");
			break;
		}
		...
		if (dp->edp_mode) {
			...
			regmap_write_bits(dp->regs, DP_VIDEO_VSAMPLE_REG, DP_STREAM_ENC_EN,
		...
		if (spacemit_dp_link_train(dp, cfg->rate, cfg->lanes) == 0) {
			dev_dbg(dp->dev, "DP: Training successful for R:%d L:%d.\n",
				cfg->rate, cfg->lanes);
			trained = true;
			break;
		}
	 ...
	 * untrained link never reaches CFG_RDY and wedges the CRTC on flip_done.
	 */
	if (!trained) {
		dev_err(dp->dev,
			"DP: no usable link config trained (sink caps: %u kHz x%u lanes); leaving video disabled\n",
			dp->link.max_rate, dp->link.max_num_lanes);
		/* The caps may be from a sink that has since been swapped out. */
		dp->link.max_rate = 0;
		dp->link.max_num_lanes = 0;

		/* Deferred: we are inside the commit and hold the modeset locks. */
		schedule_work(&dp->modeset_retry_work);
		return;
	}

	spacemit_dp_hw_set_msa_and_enable_video(dp, adjusted_mode, dp->link_rate,
...
{
	struct spacemit_dp_dev *dp = container_of(bridge, struct spacemit_dp_dev, bridge);
	struct spacemit_dp_bridge_state *st = to_spacemit_dp_bridge_state(bridge_state);
	...
	for (i = 0; i < ARRAY_SIZE(spacemit_dp_link_priority_table); i++) {
		const struct spacemit_dp_link_config *cfg = &spacemit_dp_link_priority_table[i];
...
static int spacemit_dp_edp_link_panel(struct drm_dp_aux *aux)
{
	...
	/* A non-NULL bridge.encoder means .attach already ran: late-chain panel-edp. */
	if (dp->bridge.encoder) {
		...
		/*
		 * eDP HPD-IN is unreliable, so bind may have skipped the cap read and
		 * left link.max_rate at 0. The AUX bus just worked; read them here.
		 */
		if (dp->edp_mode)
			spacemit_dp_hw_read_sink_caps(dp);
...
static irqreturn_t spacemit_dp_irq_handler(int irq, void *data)
{
	...
	new_status = spacemit_dp_hw_detect_hpd(dp);
...
static int spacemit_dp_resource_init(struct spacemit_dp_dev *dp, struct platform_device *pdev)
{
	...
	/*
	 * Only eDP0 has a mux. Test for the property: there is no devm-wrapped
	 * optional get, and a missing one logs an error and returns -ENOENT.
	 */
	if (of_property_present(dev->of_node, "mux-controls")) {
		...
		if (IS_ERR(mux))
			return dev_err_probe(dev, PTR_ERR(mux),
					     "failed to get TX-routing mux\n");

		ret = mux_control_select(mux, 1);
		if (ret)
			return dev_err_probe(dev, ret,
					     "failed to route DPU0 to eDP0 TX\n");
		mux_control_deselect(mux);
	}

	/* The pixel clock comes from the PHY PLL, so the controller never sets it. */
...
static int spacemit_dp_dev_init(struct spacemit_dp_dev *dp)
{
	...
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_PHY_RESET, FIELD_PREP(DP_PHY_RESET, 0x1));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_HDCP_RESET, FIELD_PREP(DP_HDCP_RESET, 0x1));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_AUX_RESET, FIELD_PREP(DP_AUX_RESET, 0x1));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_VIDEO_RESET, FIELD_PREP(DP_VIDEO_RESET, 0x1));
	usleep_range(5000, 8000);

	/* Clear Video Reset */
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_CONTROLLER_RESET,
			  FIELD_PREP(DP_CONTROLLER_RESET, 0x0));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_PHY_RESET, FIELD_PREP(DP_PHY_RESET, 0x0));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_HDCP_RESET, FIELD_PREP(DP_HDCP_RESET, 0x0));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_AUX_RESET, FIELD_PREP(DP_AUX_RESET, 0x0));
	regmap_write_bits(dp->regs, DP_SOFT_RESET, DP_VIDEO_RESET, FIELD_PREP(DP_VIDEO_RESET, 0x0));
	usleep_range(2000, 4000);

	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT, DP_AUX_REPLY_EVENT_INT_STA,
			  FIELD_PREP(DP_AUX_REPLY_EVENT_INT_STA, 1));

	regmap_write_bits(dp->regs, DP_CORE_CONTROL, DP_DEFAULT_FAST_LINK_TRAIN_EN,
	...
	/* Unmask Interrupts */
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_AUX_REPLY_EVENT_INT_STA_MSK,
			  FIELD_PREP(DP_AUX_REPLY_EVENT_INT_STA_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_HDCP_INT_STA_MSK,
			  FIELD_PREP(DP_HDCP_INT_STA_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_ILLEGAL_AUX_CMD_INT_STA_MSK,
			  FIELD_PREP(DP_ILLEGAL_AUX_CMD_INT_STA_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_TYPE_C_EVENT_MSK,
	...
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_SDP_INT_STA_S3_MSK,
			  FIELD_PREP(DP_SDP_INT_STA_S3_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_SDP_INT_STA_S2_MSK,
			  FIELD_PREP(DP_SDP_INT_STA_S2_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_SDP_INT_STA_S1_MSK,
			  FIELD_PREP(DP_SDP_INT_STA_S1_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_SDP_INT_STA_S0_MSK,
	...
	regmap_write_bits(dp->regs, DP_HPD_INTERRUPT_ENABLE, DP_SINK_IRQ_EVENT_MSK,
			  FIELD_PREP(DP_SINK_IRQ_EVENT_MSK, 0x0));
	regmap_write_bits(dp->regs, DP_GENERAL_INTERRUPT_MASK, DP_HPD_INT_STA_MSK,
			  FIELD_PREP(DP_HPD_INT_STA_MSK, 0x1));
	regmap_write_bits(dp->regs, DP_HPD_INTERRUPT_ENABLE, DP_HOT_PLUG_EVENT_MSK,
			  FIELD_PREP(DP_HOT_PLUG_EVENT_MSK, 0x1));
	regmap_write_bits(dp->regs, DP_HPD_INTERRUPT_ENABLE, DP_HOT_UNPLUG_EVENT_MSK,
			  FIELD_PREP(DP_HOT_UNPLUG_EVENT_MSK, 0x1));
	regmap_write_bits(dp->regs, DP_HPD_INTERRUPT_ENABLE, DP_SINK_UNPLUG_ERROR_EVENT_MSK,
...
static int spacemit_dp_bind(struct device *dev, struct device *master, void *data)
{
	...
	ret = drm_encoder_init(drm, &dp->encoder,
			       &spacemit_dp_encoder_funcs, DRM_MODE_ENCODER_NONE, NULL);
	if (ret) {
	 ...
	 * notification here makes fbdev commit a modeset that wedges on flip_done.
	 */
...
static void spacemit_dp_unbind(struct device *dev, struct device *master, void *data)
{
	struct spacemit_dp_dev *dp = dev_get_drvdata(dev);
...
static int inno_dp_probe(struct platform_device *pdev)
{
	...
	if (dp->edp_mode) {
		...
		/*
		 * OP_DETECT is required: without it drm_bridge_connector_detect()
		 * hard-codes CONNECTED for any eDP connector and fbdev wedges at bind.
		 */
		...
		/*
		 * No OP_HPD: the HPD interrupt is gated by the DP pixel clock. Leaving
		 * it out marks the connector POLL_CONNECT | POLL_DISCONNECT instead.
		 */
		dp->bridge.ops = DRM_BRIDGE_OP_DETECT | DRM_BRIDGE_OP_EDID |
				 DRM_BRIDGE_OP_MODES;
	}
	...
	/*
	 * The PHY child shares this device's regmap, so the regmap must exist before
	 * devm_of_platform_populate() lets the PHY probe and devm_phy_get() succeed.
	 */
	...
	/* The PHY exposes its PLL as the APMU pixel-clock mux's external parent. */
	...
	/* Panel discovery needs the AUX channel here; registration waits for bind. */
	...
	/*
	 * eDP hands panel power, enable-GPIO and backlight to panel-edp over the
	 * AUX bus. A DP instance has no aux-bus child and takes the -ENODEV path.
	 */
	if (dp->edp_mode) {
	...
	/* drm_bridge_attach() expects the bridge to already be on the global list. */
	...
	/* The handler needs bind-created state, so keep the line disabled until bind. */
