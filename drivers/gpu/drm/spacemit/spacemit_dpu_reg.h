 ...
 *
 * Saturn DPU register accessors. These are function-like macros that evaluate
 * their arguments more than once; callers must pass side-effect-free expressions.
 */
...
{									\
	if ((__mask) == 0xffU)						\
		writeb((u8)(data), __addr);				\
	else if ((__mask) == 0xff00U)					\
		writeb((u8)(data), (__addr) + 1);			\
	else if ((__mask) == 0xff0000U)					\
		writeb((u8)(data), (__addr) + 2);			\
	else if ((__mask) == 0xff000000U)				\
		writeb((u8)(data), (__addr) + 3);			\
	else if ((__mask) == 0xffffU)					\
		writew((u16)(data), __addr);				\
	else if ((__mask) == 0xffff0000U)				\
		writew((u16)(data), (__addr) + 2);			\
...
{									\
	if ((cl_regs) != NULL)						\
		...
		dpu_write_reg(hwdev, module_name, module_base, field, data) \
}
