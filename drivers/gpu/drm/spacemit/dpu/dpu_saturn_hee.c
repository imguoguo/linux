...
static const u32 PREPIPE_SCAL_BASE_ADDR[] = {
	...
	if (color_encoding != old_state->color_encoding || color_range != old_state->color_range) {
		...
		for (i = 0; i < 12; i += 2, index++) {
			value = (spacemit_yuv2rgb_coefs[color_encoding][color_range][i] & 0x3FFF) |
				((spacemit_yuv2rgb_coefs[color_encoding][color_range][i + 1] &
				  0x3FFF) << 16);
			dpu_write_word(hwdev, module_base, index, value,
				       cl_rdma);
		}
	}
}
...
void saturn_hee_conf_scaler_x(struct drm_plane_state *state, struct cmdlist_regs *cl)
{
	...
	u32 hor_delta_phase, ver_delta_phase;
	s64 hor_init_phase, ver_init_phase;
	u32 hor_init_phase_h1b, ver_init_phase_h1b;
	...
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_enable, hor_enable, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_enable, ver_enable, cl_scl);

	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_delta_phase, hor_delta_phase,
		  cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_delta_phase, ver_delta_phase,
		  cl_scl);

	hor_init_phase = ((int64_t)hor_delta_phase - 65536) >> 1;
	ver_init_phase = ((int64_t)ver_delta_phase - 65536) >> 1;

	drm_dbg(state->plane->dev, "hor_delta:%d ver_delta:%d\n", hor_delta_phase, ver_delta_phase);
	drm_dbg(state->plane->dev, "hor_init_phase:%lld ver_init_phase:%lld\n", hor_init_phase,
		ver_init_phase);

	if (hor_init_phase >= 0) {
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_init_phase_l32b,
			  hor_init_phase & 0x00000000ffffffff, cl_scl);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_init_phase_h1b, 0, cl_scl);
	} else { /* convert to positive value if negative */
		hor_init_phase += 8589934592;
		hor_init_phase_h1b = (u32)((hor_init_phase & 0x0000000100000000ULL) >> 32);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_init_phase_l32b,
			  hor_init_phase & 0x00000000ffffffff, cl_scl);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_init_phase_h1b,
			  hor_init_phase_h1b, cl_scl);
	}

	if (ver_init_phase >= 0) {
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_init_phase_l32b,
			  ver_init_phase & 0x00000000ffffffff, cl_scl);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_init_phase_h1b, 0, cl_scl);
	} else { /* convert to positive value if negative */
		ver_init_phase += 8589934592;
		ver_init_phase_h1b = (u32)((ver_init_phase & 0x0000000100000000ULL) >> 32);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_init_phase_l32b,
			  ver_init_phase & 0x00000000ffffffff, cl_scl);
		dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_init_phase_h1b,
			  ver_init_phase_h1b, cl_scl);
	}

	/* enable both ver and hor */
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_hor_enable, 0x1, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_ver_enable, 0x1, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_input_width, in_width, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_input_height, in_height, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_output_width, out_width, cl_scl);
	dpu_write(hwdev, SCALER_X_REG, module_base, b_m_nscl_output_height, out_height, cl_scl);
...
void saturn_hee_cfg_ready(struct spacemit_crtc *a_crtc, struct spacemit_hw_device *hwdev)
{
	...
	saturn_enable_irq_mask(a_crtc, true, DPU_INT_BASE_ADDR + DPU_ONLINE_IRQ_MSK,
...
static void saturn_init_csc(struct spacemit_crtc *a_crtc)
{
	...
	struct spacemit_hw_device *hwdev = priv->hwdev;
	const s16 *coef =
		spacemit_yuv2rgb_coefs[DRM_COLOR_YCBCR_BT709][DRM_COLOR_YCBCR_LIMITED_RANGE];
	u32 module_base;
	int i = 0;
	...
	for (i = 0; i < hwdev->rdma_nums; i++) {
		...
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix00,
			  coef[0] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix01,
			  coef[1] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix02,
			  coef[2] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix03,
			  coef[3] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix10,
			  coef[4] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix11,
			  coef[5] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix12,
			  coef[6] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix13,
			  coef[7] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix20,
			  coef[8] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix21,
			  coef[9] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix22,
			  coef[10] & 0x3FFF);
		dpu_write(hwdev, RDMA_PATH_X_REG, module_base, csc_matrix23,
			  coef[11] & 0x3FFF);
	}
}
...
static void saturn_init_tmg(struct spacemit_crtc *a_crtc)
{
	...
	/* dev_id names the composer type, not the DPU instance: use the base. */
...
void saturn_hee_irq_enable(struct spacemit_crtc *a_crtc, bool enable)
{
	...
	if (enable) {
		...
		/* Ack first: a surviving status bit would fire as the mask opens. */
...
{\
	...
	if (unlikely(solid_en)) { \
		...
		dpu_write(hwdev, CMPS_X_REG, base, layer ## id_name ## _solid_en,	\
			  1, cl_cmp);				\
		pr_debug("solid_r:0x%x, solid_g:0x%x, solid_b:0x%x, solid_a:0x%x\n", \
...
static void dpu_saturn_scaler_reuse_en(struct drm_plane *plane, bool enable)
{
	...
	switch (rdma_id) {
	case DPU_RDMA0:
		dpu_write(hwdev, DPU_CTL_TOP_REG, DPU_CTRL_BASE_ADDR, nml_scl0_reuse_en, enable,
			  cl_dpuctl);
		break;
	case DPU_RDMA1:
		dpu_write(hwdev, DPU_CTL_TOP_REG, DPU_CTRL_BASE_ADDR, nml_scl1_reuse_en, enable,
			  cl_dpuctl);
		break;
	case DPU_RDMA2:
		dpu_write(hwdev, DPU_CTL_TOP_REG, DPU_CTRL_BASE_ADDR, nml_scl2_reuse_en, enable,
			  cl_dpuctl);
		break;
	case DPU_RDMA3:
		dpu_write(hwdev, DPU_CTL_TOP_REG, DPU_CTRL_BASE_ADDR, nml_scl3_reuse_en, enable,
			  cl_dpuctl);
		break;
	default:
		drm_err(plane->dev, "Unsupported RDMA id for scaler!\n");
	}
...
void saturn_hee_plane_update_hw_channel(struct drm_plane *plane, int slice_id)
{
	...
	drm_dbg(plane->dev, "crtc_x %u crtc_x %u\n", crtc_x, crtc_y);
	...
	if (!solid_en) {
		...
		dpu_write(hwdev, RDMA_PATH_X_REG, base, layer_cmpsr_id, a_crtc->dev_id, cl_rdma);
		/* force online-mode routing whatever the bootloader left behind */
		dpu_write(hwdev, RDMA_PATH_X_REG, base, is_offline, 0, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, is_two_layers, 0, cl_rdma);

		dpu_write(hwdev, RDMA_PATH_X_REG, base, img_width_ly0, fb->width, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, img_height_ly0, fb->height, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, bbox_start_x_ly0, src_x, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, bbox_start_y_ly0, src_y, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, bbox_end_x_ly0, src_x + src_w - 1, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, bbox_end_y_ly0, src_y + src_h - 1, cl_rdma);
		...
		dpu_write(hwdev, RDMA_PATH_X_REG, base, rot_mode_ly0, val, cl_rdma);
		if (fccf == DRM_FORMAT_YVYU || fccf == DRM_FORMAT_VYUY ||
		    fccf == DRM_FORMAT_NV21 || fccf == DRM_FORMAT_YVU420 ||
		    fccf == DRM_FORMAT_Q401)
			uv_swap = 1;

		dpu_write(hwdev, RDMA_PATH_X_REG, base, uv_swap, uv_swap, cl_rdma);
		dpu_write(hwdev, RDMA_PATH_X_REG, base, pixel_format, spacemit_plane_state->format,
			  cl_rdma);
	...
	case 0:
		...
		break;
	case 1:
		SATURN_WRITE_COMPOSER_LAYER(01);
		break;
	case 2:
		SATURN_WRITE_COMPOSER_LAYER(02);
		break;
	case 3:
		SATURN_WRITE_COMPOSER_LAYER(03);
		break;
	case 4:
		SATURN_WRITE_COMPOSER_LAYER(04);
		break;
	case 5:
		SATURN_WRITE_COMPOSER_LAYER(05);
		break;
	case 6:
		SATURN_WRITE_COMPOSER_LAYER(06);
		break;
	case 7:
		SATURN_WRITE_COMPOSER_LAYER(07);
		break;
	case 8:
		SATURN_WRITE_COMPOSER_LAYER(08);
		break;
	case 9:
		SATURN_WRITE_COMPOSER_LAYER(09);
		break;
	case 10:
		SATURN_WRITE_COMPOSER_LAYER(10);
		break;
	case 11:
		SATURN_WRITE_COMPOSER_LAYER(11);
		break;
	case 12:
		SATURN_WRITE_COMPOSER_LAYER(12);
		break;
	case 13:
		SATURN_WRITE_COMPOSER_LAYER(13);
		break;
	case 14:
		SATURN_WRITE_COMPOSER_LAYER(14);
		break;
	case 15:
		SATURN_WRITE_COMPOSER_LAYER(15);
		break;
	default:
		drm_err(plane->dev, "%s unsupported zpos:%d\n", __func__, state->zpos);
		break;
	}
...
void saturn_hee_plane_disable_hw_channel(struct drm_plane *plane, struct drm_plane_state *old_state)
{
	...
	case 0:
		...
		break;
	case 1:
		dpu_write(hwdev, CMPS_X_REG, base, layer01_en, 0, cl_cmp);
		break;
	case 2:
		dpu_write(hwdev, CMPS_X_REG, base, layer02_en, 0, cl_cmp);
		break;
	case 3:
		dpu_write(hwdev, CMPS_X_REG, base, layer03_en, 0, cl_cmp);
		break;
	case 4:
		dpu_write(hwdev, CMPS_X_REG, base, layer04_en, 0,  cl_cmp);
		break;
	case 5:
		dpu_write(hwdev, CMPS_X_REG, base, layer05_en, 0,  cl_cmp);
		break;
	case 6:
		dpu_write(hwdev, CMPS_X_REG, base, layer06_en, 0,  cl_cmp);
		break;
	case 7:
		dpu_write(hwdev, CMPS_X_REG, base, layer07_en, 0, cl_cmp);
		break;
	case 8:
		dpu_write(hwdev, CMPS_X_REG, base, layer08_en, 0, cl_cmp);
		break;
	case 9:
		dpu_write(hwdev, CMPS_X_REG, base, layer09_en, 0, cl_cmp);
		break;
	case 10:
		dpu_write(hwdev, CMPS_X_REG, base, layer10_en, 0, cl_cmp);
		break;
	case 11:
		dpu_write(hwdev, CMPS_X_REG, base, layer11_en, 0, cl_cmp);
		break;
	case 12:
		dpu_write(hwdev, CMPS_X_REG, base, layer12_en, 0, cl_cmp);
		break;
	case 13:
		dpu_write(hwdev, CMPS_X_REG, base, layer13_en, 0, cl_cmp);
		break;
	case 14:
		dpu_write(hwdev, CMPS_X_REG, base, layer14_en, 0, cl_cmp);
		break;
	case 15:
		dpu_write(hwdev, CMPS_X_REG, base, layer15_en, 0, cl_cmp);
		break;
	default:
		drm_err(plane->dev, "%s unsupported zpos:%d %d\n", __func__, old_state->zpos,
			p->hw_pid);
		break;
	}
...
static u32 saturn_conf_dpuctrl_scaling(struct spacemit_crtc *a_crtc)
{
	...
	for (i = 0; i < MAX_SCALER_NUMS; i++) {
		scaler = &ac->scalers[i];
		drm_dbg(a_crtc->crtc.dev, "scaler%d: in_use:0x%x rdma_id:%d\n", i, scaler->in_use,
			scaler->rdma_id);
	...
	drm_dbg(a_crtc->crtc.dev, "scl_en:%d\n", scl_en);
			     ...
			     struct drm_crtc_state *old_state)
{
	...
	/* postpipe should be configured no matter whether pq function is on or off */
	dpu_write_word(hwdev, pp_base, 0,
		       a_crtc->split_en ? 0x20 : 0, pp_cl);
	dpu_write(hwdev, POSTPIPE_REG, pp_base, m_inwidth, mode->hdisplay, pp_cl);
	dpu_write(hwdev, POSTPIPE_REG, pp_base, m_inheight, mode->vdisplay, pp_cl);
...
u32 saturn_hee_get_cfg_rdy(struct spacemit_crtc *a_crtc, struct spacemit_hw_device *hwdev)
{
	u32 base = DPU_SCENE_CTL_ADDR(a_crtc->dev_id);
			  ...
			  struct drm_plane *plane)
{
	...
	base = RDMA_BASE_ADDR[rdma_id];
	dpu_write(hwdev, RDMA_PATH_X_REG, base, rdma_stride0_layer0, fb->pitches[0], cl_rdma);
	dpu_write(hwdev, RDMA_PATH_X_REG, base, rdma_stride1_layer0, fb->pitches[1], cl_rdma);
				      ...
				      u8 dev_id)
{
	...
	if (cl->type == CMDLIST_PLANE) {
		...
		if (next_crtc_y != -1 && cl_to_spacemit_pstate(cl->next) != spacemit_pstate &&
			...
			pr_err_ratelimited("Invalid gap: cur_crtc_y:%d cur_crtc_h:%d next_crtc_y:%d\n",
	...
	if (size > PER_CMDLIST_SIZE)
		pr_warn_ratelimited("plane%d cmdlist occupies %d bytes!\n", zpos, size);
}
			     ...
			     struct spacemit_drm_private *priv, u8 crtc_id, u8 dev_id)
{
	...
	dpu_write_reg_arr(hwdev, CMDLIST_REG, CMDLIST_BASE_ADDR, cmdlist_reg_48,
			  hwdev->rdma_nums + 2 + crtc_id, cmdlist_ch_y_first, val);

	val = ((priv->cmdlist_groups[hwdev->rdma_nums + crtc_id]->pa) &
	       CMDLIST_ADDRL_ALIGN_MASK) >> CMDLIST_ADDRL_ALIGN_BITS;
	dpu_write_reg_arr(hwdev, CMDLIST_REG, CMDLIST_BASE_ADDR, cmdlist_reg_0,
			  hwdev->rdma_nums + 2 + crtc_id, cmdlist_ch_start_addrl, val);
	...
	dpu_write_reg_arr(hwdev, CMDLIST_REG, CMDLIST_BASE_ADDR, cmdlist_reg_16,
			  hwdev->rdma_nums + 2 + crtc_id, cmdlist_ch_start_addrh, val);
	dpu_write_reg_arr(hwdev, DPU_CTL_TOP_REG, DPU_CTRL_BASE_ADDR,
	...
	for (int i = 0; i < header->nod_len - 1; i++) {
		...
		for (int j = 0; j < CMDLIST_ROW_REGS; j++) {
			if (row->module_cfg_strobe & CMDLIST_REG_STROBE(j))
				ret += sprintf(tmp + ret, "[%#10x:%#10x]", addr + j * 4,
					       row->module_regs[j]);
			else
				ret += sprintf(tmp + ret, "%#11x:%#11x", addr + j * 4,
					       row->module_regs[j]);
		}
		trace_plat_cmdlist_dump_node(tmp);
	}
